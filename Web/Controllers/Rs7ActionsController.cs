...
namespace MoE.ECE.Web.Controllers
{
    ...
    public class Rs7ActionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<Rs7NewRequestModel>> PostActionNewRequest([FromBody] CreateSkeletonRs7 request,
            CancellationToken cancellationToken)
        {
            int id = await _cqrs.ExecuteAsync(request, cancellationToken);

            Rs7NewRequestModel? newRequest = _mapper.Map<Rs7NewRequestModel>(request);
            newRequest.Rs7Id = id;
        ...
        public async Task<ActionResult> PostActionDraftChanges([FromRoute] int id,
            [FromBody] SaveRs7EntitlementDetailsDraft request, CancellationToken cancellationToken) =>
        ...
        public async Task<ActionResult> PutEntitlementMonth([FromRoute] int id, [FromRoute] int monthNumber,
            [FromBody] UpdateRs7EntitlementMonth request, CancellationToken cancellationToken)
        {
            request.Id = id;
        ...
        ///     This action updates the RS7 roll data and starts a workflow for the two step approval process
        /// </remarks>
        /// <returns></returns>
        [HttpPost("{id}/submissions-for-approval")]
        [RequirePermission(Rs7.SubmissionsForApprovalCreate)]
        public async Task<ActionResult> PostActionSubmitForApproval([FromRoute] int id,
            [FromBody] SubmitRs7ForApproval request, CancellationToken cancellationToken)
        {
            request.Id = id;
            await _cqrs.ExecuteAsync(request, cancellationToken);
            return NoContent();
        ...
        /// <remarks>
        ///     Experimental
        ///     Allows a user with dual role or an internal user to create and submit an RS7 created by an external user to the
        ///     ministry
        ...
        public async Task<ActionResult> PostActionSubmit([FromRoute] int id, [FromBody] SubmitRs7 request,
            CancellationToken cancellationToken) => await Task.FromResult(NoContent());
        ...
        public async Task<ActionResult> PutDeclaration([FromRoute] int id, [FromBody] UpdateRs7Declaration request,
            CancellationToken cancellationToken)
        {
            request.Id = id;
            await _cqrs.ExecuteAsync(request, cancellationToken);
        ...
        public async Task<ActionResult> PostActionZeroReturn([FromRoute] CreateSkeletonRs7 request,
            CancellationToken cancellationToken) => await Task.FromResult(NoContent());
    }
}