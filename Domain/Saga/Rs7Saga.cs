...
namespace MoE.ECE.Domain.Saga
{
        ...
        IHandleACommand<UpdateRs7Declaration>
    {
        ...
        public async Task<int> Handle(CreateRs7ZeroReturn command, CancellationToken cancellationToken)
        {
            ...
            Rs7? rs7 = await LoadOrCreateRs7(command.OrganisationId.GetValueOrDefault(), command.FundingPeriod,
                command.FundingPeriodYear, cancellationToken);
        ...
        {
            Rs7? rs7 = await _documentSession.LoadRs7Async(command.Id, cancellationToken);
            ...
            Rs7Discarded? domainEvent = new Rs7Discarded(rs7, _systemClock.UtcNow);
        ...
        public Task<Unit> Handle(SaveAsDraft command, CancellationToken cancellationToken) =>
            DoUpdateRs7(command, cancellationToken);
        ...
        private async Task<Unit> DoUpdateRs7<TSource>(TSource command, CancellationToken cancellationToken)
            where TSource : Rs7Model
        {
            Rs7? rs7 = await _documentSession.LoadRs7Async(command.Id, cancellationToken);
        ...
        ///     Thrown when the service doesn't exist, or when license checks performed and the
        ///     license is not eligible
        /// </exception>
        private void ServiceAndEligibilityCheck(int organisationId, bool checkLicense)
        {
            EceService? eceService = _refDataContext.EceServices.FindNullable(organisationId);
            if (eceService == null)
            {
            ...
            if (checkLicense
                && (eceService.LicenceStatusId == LicenceStatus.Cancelled ||
                    eceService.LicenceStatusId == LicenceStatus.Suspended))
            {
                throw EceServiceIneligibleBecauseLicenceStatus(eceService.OrganisationName,
                    eceService.RefOrganisationId,
        ...
        private async Task<Rs7> LoadOrCreateRs7(int organisationId, FundingPeriodMonth fundingPeriod,
            int fundingPeriodYear, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    throw DuplicateResourceFundingPeriod<Rs7>(organisationId, fundingPeriod.ToString(),
                        fundingPeriodYear);
                }
            }
        ...
        private async Task SaveAndRaiseAsync<TDomainEvent>(Rs7 rs7, CancellationToken cancellationToken)
            where TDomainEvent : IDomainEvent
        {
            await _documentSession.SaveChangesAsync(cancellationToken);

            TDomainEvent? domainEvent = _mapper.Map<TDomainEvent>(rs7);
