...
namespace MoE.ECE.Domain.Saga
{
        ...
        IHandleACommand<DeclineRs7>
    {
        ...
        public Task<Unit> Handle(ApproveRs7 command, CancellationToken cancellationToken) =>
            ApplyStateChange<Rs7Approved>(command.BusinessEntityId, rs7 => rs7.ApproveInternally(_systemClock.UtcNow),
                cancellationToken);

        public Task<Unit> Handle(DeclineRs7 command, CancellationToken cancellationToken) =>
            ApplyStateChange<Rs7Declined>(command.BusinessEntityId, rs7 => rs7.Decline(_systemClock.UtcNow),
                cancellationToken);

        public Task<Unit> Handle(PeerApproveRs7 command, CancellationToken cancellationToken) =>
            ApplyStateChange<Rs7PeerApproved>(command.BusinessEntityId, rs7 => rs7.PeerApprove(_systemClock.UtcNow),
                cancellationToken);

        public Task<Unit> Handle(Rs7PeerReject command, CancellationToken cancellationToken) =>
            ApplyStateChange<Rs7PeerRejected>(command.BusinessEntityId, rs7 => rs7.PeerReject(_systemClock.UtcNow),
                cancellationToken);
        ...
        public async Task<Unit> Handle(SubmitRs7ForApproval command, CancellationToken cancellationToken)
        {
            ...
            Rs7SubmittedForApproval? domainEvent = _mapper.Map<Rs7SubmittedForApproval>(rs7);
        ...
        {
            Rs7? rs7 = await _documentSession.LoadRs7ByBusinessEntityIdAsync(businessEntityId, cancellationToken);
